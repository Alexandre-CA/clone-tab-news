...
export async function GET() {
  ...
  const dbMaxConnection = (await database.query("SHOW max_connections")).rows[0];

  // Query the database for the used connections
  const usedConnections = (await database.query("SELECT COUNT(*)::int FROM pg_stat_activity WHERE state='active';")).rows[0];
